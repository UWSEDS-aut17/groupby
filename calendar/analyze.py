...
def _total_minutes(seconds):
  """Converts seconds to minutes.

  Args:
      seconds: the total seconds
  Returns:
      the total minutes as a float
  """
  return seconds / 60.0
...
def _parse_regexes(regexes):
  ...
  print("Regex parser output", results)
  return results
...
def _process_calendar(calendar_file,
  ...
  cal_events = pd.DataFrame(columns=['event_name','duration','count'])
  ...
  for event in calendar.events:
    ...
    #cal_events.loc[i] = [event.name, _total_minutes(event.duration.total_seconds()), 1]
    cal_events.loc[i] = [event.name, _total_minutes(event.duration.total_seconds), 1]
