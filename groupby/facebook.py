...
def clean_timeline(filename):
    ...
    days = pd.DataFrame.from_dict(days, orient='index').reset_index().rename(
        columns={'index': 'Days', 0: 'Count'})
...
def clean_friends(filename):
    ...
    for i in clean_friends:
        clean_new_timeline.append(
            [x.strip() for x in i.split('Sent Friend Requests')])
    ...
    for l in range(len(the_new_list)):
        ...
        if len(k[0]) == 2:
            k[0].append('2017')
        new.append(k[0])
    clean_df = pd.DataFrame(new).rename(
        columns={0: 'Date', 1: 'Month', 2: 'Year'})
