...
def build_report(user_args):
    ...
    try:
        ...
        if user_args.linkedin is not None:
            ...
            connections = linkedin.plot(con_df_by_week, 'Connected On',
                                        'count', 'Weeks',
                                        'Number of Connections',
                                        'Number of Connections per week',
                                        (15, 5), '#8D6CAB')
            connections.savefig('connections.png')
            pdf.image('connections.png', w=8.5, x=0)
            subprocess.call(['rm', 'connections.png'])

            invites_sent, invites_received = linkedin.get_sent_receive_invites(
                invites_df, 'Direction')
            invites_sent_by_week = linkedin.clean_df(invites_sent, 'Sent At')
            inv_sent = linkedin.plot(invites_sent_by_week, 'Sent At', 'count',
                                     'Weeks', 'Number of Invites',
                                     'Number of Invites Sent per week',
                                     (15, 5), '#8D6CAB')
            inv_sent.savefig('inv_sent.png')
            pdf.image('inv_sent.png', w=8.5, x=0)
            subprocess.call(['rm', 'inv_sent.png'])

            invites_received_by_week = linkedin.clean_df(invites_received,
                                                         'Sent At')
            inv_rec = linkedin.plot(invites_received_by_week, 'Sent At',
                                    'count', 'Weeks', 'Number of Invites',
                                    'Number of Invites Received per week',
                                    (15, 5), '#8D6CAB')
        ...
        if user_args.twitter is not None:
            ...
            top_5_hashtags = plotters.plot(hashtags, values, hashtags_int,
                                           'hashtags', 'Number',
                                           'Top 5 Tweet Hashtags', (15, 5),
                                           'Green', '-T')
            top_5_hashtags.savefig('top_5_hashtags.png')
            pdf.image('top_5_hashtags.png', w=7)
            subprocess.call(['rm', 'top_5_hashtags.png'])

            friends_list, friends_int, m_values = twitter.mentions_clean(
                tweets_df)
            top_mentions = plotters.plot(friends_list, m_values, friends_int,
                                         'Friend', 'Number of Mentions',
                                         'Top 5 Friend Mentions', (15, 5),
                                         'Green', '-T')
            ...
            tweets_per_month = plotters.plot_tweetDate(month_df, labels,
                                                       'Month',
                                                       'Number of Tweets',
                                                       'Total Tweets Per '
                                                       'Month',
            ...
            try:
                scores_dict = twitter.sentiment_dict(
                    './groupby/data/AFINN-111.txt')
            except:
        ...
        if user_args.facebook is not None:
            ...
            days, month, year, fb_df = facebook.clean_timeline(
                fb_timeline_fname)

            activity_per_day = facebook.plot(days, 'Days', 'Count',
                                             'Day Of Week', 'Activity count',
                                             'Timeline Activity Across Days',
                                             (15, 5), 'purple')
            ...
            activity_per_month = facebook.plot(month, 'Date', 'Count', 'Month',
                                               'Activity count',
                                               'Timeline Actvity Across '
                                               'Months',
                                               (15, 5), 'blue')
            ...
            activity_per_year = facebook.plot(year, 'Date', 'Count', 'Year',
                                              'Activity count',
                                              'Timeline Activity Across the '
                                              'Years',
                                              (15, 5), 'red')
            ...
            pdf.set_margins(1, 1)
            pdf.cell(w=0, h=.3, txt='', align='C', ln=1)
            friends_per_year = facebook.plot(friends_df, 'Year', 'Date',
                                             'Year', 'New friends count',
                                             'New Friends By Year', (15, 5),
                                             'red')
        ...
        if user_args.calendar is not None:
            ...
            time_per_month, events_per_month, total_events = gcal.get_plots(
                cal_df)
        ...
        if user_args.twitter and user_args.facebook and user_args.linkedin:
            ...
            joined_df = tweets_df_month.merge\
                (conn_df_month, left_on='index',
                 right_on='index',
                 how='outer').merge(gcal_df_month,
                                    left_on='index',
                                    right_on='index',
                                    how='outer').merge(
                 fb_df_month, left_on='index', right_on='index',
...
def main():
    parser = argparse.ArgumentParser(
        description='Analyzes social media events to track time spent on '
                    'activities.')
    ...
    parser.add_argument('-O',
                        '--output',
                        help=(
                            'The directory where the report should be '
                            'located, e.g. ~/Desktop')
                        )
